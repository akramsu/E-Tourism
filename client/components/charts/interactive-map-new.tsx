...
export function InteractiveMap({
  ...
  const generateFallbackCountryData = (): CountryData[] => {
    return [
      ...
      { country: "Canada", count: 790, percentage: 2.0, iso: "CAN" },
      { country: "Netherlands", count: 395, percentage: 1.0, iso: "NLD" }
    ]
  }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isAuthorityContext) {
          if (showCityWideData) {
            data = await authorityApi.getCityDemographics({ period })
          } else if (attractionId) {
            data = await authorityApi.getAttractionDetails(attractionId)
          } else {
          ...
          data = await ownerApi.getVisitorAnalytics(attractionId, { period })
        }

        if (data?.success && data?.data) {
          const apiData = data.data
          let locationData = apiData.locationBreakdown || apiData.demographics?.location || []
          
          if (Array.isArray(locationData)) {
            const total = locationData.reduce((sum: number, item: any) => sum + item.count, 0)
            setTotalVisitors(total)

            const countries = locationData
              .filter((item: any) => item.country && item.country !== 'Unknown')
              .map((item: any) => ({
